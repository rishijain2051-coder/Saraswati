// Safe arithmetic expression evaluator (mirror of server/src/lib/expr.ts).
...
function evalRPN(rpn: Tok[], vars: Record<string, number>): number {
  ...
  for (const tk of rpn) {
    if (tk.t === 'num') st.push(tk.v);
      ...
      if (tk.v === 'u-') { const a = st.pop() ?? 0; st.push(-a); continue; }
      const b = st.pop();
      const a = st.pop();
